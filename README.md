...
- **スキャナ（カテゴリ）**
  ...
  - `winjunk` : Windows定番の不要ファイル（一時・サムネイル/シェーダーキャッシュ・
    クラッシュログ等。ユーザー領域のみ・管理者権限不要・低リスク）
  - `large` : 大容量ファイル（既定 100MB以上）
  - `duplicates` : 内容が同一の重複ファイル（1つを残して提案）
  - `stale` : 長期間アクセスのないファイル（既定 180日）
  - `largeapps` : 大容量アプリ（既定 500MB以上・**提示のみ／自動削除しない**）
  - `apps` : 最近使用されていないアプリ（使用実績が古いもののみ提案・
    **検出と削除方法の提示のみ／自動削除はしない**）
- **確認と安全機構**
